...
# 1. Initialize Database for Dynamic Mentors & Tracks
def init_db():
...
# 2. Real GitHub Validation Function (Errors out on fake/invalid links)
def validate_github_profile_and_repo(profile_url):
  if not profile_url:
  ...
  try:
    # Check for User Profile or Repository Link
    if len(parts) == 4:  # Format: github.com/username
      ...
      if res.status_code == 200:
        ...
        return {
            "valid": False,
            "error": (
                "GitHub repository does not exist, is private, or URL is"
                " incorrect."
            ),
        }
  except Exception as e:
...
# 3. Dynamic Database Functions (No Dummy Data)
def add_mentor(name, expertise):
  conn = sqlite3.connect("internship_system.db")
  cursor = conn.cursor()
  cursor.execute(
      "INSERT INTO mentors (name, expertise) VALUES (?, ?)", (name, expertise)
  )
  conn.commit()
  conn.close()
...
def get_dynamic_mentors():
  conn = sqlite3.connect("internship_system.db")
  cursor = conn.cursor()
  cursor.execute("SELECT name, expertise FROM mentors")
  mentors = cursor.fetchall()
  conn.close()
  # Agar database khali ho toh fallback message dikhayein taake error na aaye
  if not mentors:
    return [("No Mentors Added Yet", "Please add via Admin Panel")]
  return mentors
...
# ---------------------------------------------------
            ...
            if resume_file is not None:
                with st.spinner("Parsing and Analyzing Resume PDF..."):
                    ...
                    if not pdf_text.startswith("Error"):
                        extracted_text += ", " + pdf_text[:300] # Append extracted text
                        st.success(f"✅ **Resume Uploaded & Parsed:** {resume_file.name} ({round(resume_file.size / 1024, 2)} KB)")
                    else:
    ...
    with tab2:
        ...
        if st.button("Fetch Recommendations & AI Reasoning"):
            ...
            if recommendations:
                ...
                user_skills_list = [s.strip().lower() for s in user_skills.lower().split(',')]
                ...
                assigned_mentor = get_dynamic_mentor_recommendation(user_skills, job_domain)
    ...
    with tab4:
        ...
        st.success("Application is fully synchronized with database-driven internship recommendations, PDF parsing, AI reasoning, and deployed successfully!")
        
        st.markdown("### 🗺️ Personalized Learning Roadmap")
        sample_missing = ["PyTorch Advanced Optimization", "Deployment with Streamlit Cloud"]

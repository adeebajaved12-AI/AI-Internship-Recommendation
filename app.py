...
if not st.session_state.logged_in:
  st.markdown(
      "<div class='brand-logo'>EZITECH PORTAL</div>", unsafe_allow_html=True
  )
  st.markdown(
      "<div class='main-title'>🔐 Portal Authentication</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
      "<div class='sub-title'>Secure access control for Students, Mentors, and"
      " Administrators.</div>",
      unsafe_allow_html=True,
  )
  ...
  with col1:
    st.markdown(
        "<div class='section-title'>Select Options</div>", unsafe_allow_html=True
    )
  ...
  with col2:
    if auth_mode == "Login":
      st.markdown(
          f"<div class='section-title'>{role} Portal Login</div>",
          unsafe_allow_html=True,
      )
      ...
      st.markdown(
          f"<div class='section-title'>{role} Registration</div>",
          unsafe_allow_html=True,
      )
...
# ---------------------------------------------------
  ...
  st.sidebar.title(
      f"Welcome, {st.session_state.get('user_name', 'Adeeba')}"
  )
  ...
  st.markdown(
      "<div class='brand-logo'>EZITECH INTERNSHIP PLATFORM</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
      "<div class='main-title'>🚀 Intelligent Internship Recommendation &"
      " Matching Engine</div>",
      unsafe_allow_html=True,
  )
  st.markdown(
      "<div class='sub-title'>Candidate Multi-Source Application Portal &"
      " Verification Dashboard</div>",
      unsafe_allow_html=True,
  )
  ...
  with tab1:
    st.markdown(
        "<div class='section-title'>Candidate Multi-Source Application"
        " Portal</div>",
        unsafe_allow_html=True,
    )
    ...
    portfolio_url = st.text_input(
        "🌐 Enter Portfolio or Live Website URL (Optional):"
    )
    ...
    if st.button("Run Real-Time Multi-Source Analysis"):
      ...
      if resume_file is not None:
        with st.spinner("Parsing and Analyzing Resume PDF..."):
          ...
          if not pdf_text.startswith("Error"):
            ...
            st.success(
                f"✅ **Resume Uploaded & Parsed:** {resume_file.name}"
                f" ({round(resume_file.size / 1024, 2)} KB)"
            )
          else:
      ...
      if github_url:
        with st.spinner("Analyzing GitHub Profile/Repo via API..."):
          ...
          if val_result.get("valid"):
            st.success(
                f"✅ **GitHub Verified:** Type: {val_result.get('type')} |"
                " Lang/Name:"
                f" {val_result.get('name') or val_result.get('language')}"
            )
          else:
            st.warning(
                "⚠️ **GitHub Notice:**"
                f" {val_result.get('error', 'Could not fully verify repository.')}"
            )
      ...
      if portfolio_url:
        with st.spinner("Checking Portfolio Link..."):
          if portfolio_url.startswith("http://") or portfolio_url.startswith(
            ...
            st.success(
                f"✅ **Portfolio Connected:** {portfolio_url} is active and"
                " ready."
            )
          else:
      ...
      st.success(
          "🎯 **Analysis Complete!** Switch to the **'Match, Classifier &"
          " Recommendations'** tab to view your matched internships."
      )

  with tab2:
    st.markdown(
        "<div class='section-title'>AI Classifier & Internship"
        " Recommendations</div>",
        unsafe_allow_html=True,
    )
    st.info(
        "Based on your uploaded Resume and GitHub analysis, here are the best"
        " matching internships tailored for your profile:"
    )

    user_skills = st.text_input(
        "Detected Skills / Enter Custom Skills:",
        value=st.session_state.get("extracted_skills", "Python, AI"),
    )
    ...
    if st.button("Fetch Recommendations & AI Reasoning"):
      ...
      if recommendations:
        ...
        job_title, job_domain, job_desc, job_skills = (
            top_match[0],
            top_match[1],
            top_match[2],
            top_match[3],
        ...
        user_skills_list = [
            s.strip().lower() for s in user_skills.lower().split(",")
        ]
        matched_count = sum(
            1
            for rs in req_skills_list
            if any(rs.lower() in us for us in user_skills_list)
        )

        score, reasoning = generate_ai_reasoning(
            req_skills_list[: max(matched_count, 1)], job_skills
        )
        assigned_mentor = get_dynamic_mentor_recommendation(
            user_skills, job_domain
        )
        ...
        with col_a:
          ...
          st.metric(label="AI Confidence Match Score", value=f"{score}%")
  ...
  with tab3:
    st.markdown(
        "<div class='section-title'>Mentor Database & Allocation</div>",
        unsafe_allow_html=True,
    )
    ...
    if mentors:
      for m in mentors:
        st.write(
            f"**Name:** {m[0]} | **Expertise:** {m[1]} | **Domain:** {m[2]} |"
            f" **Contact:** {m[3]}"
        )
    else:
  ...
  with tab4:
    st.markdown(
        "<div class='section-title'>Roadmap & Final Deployment Status</div>",
        unsafe_allow_html=True,
    )
    st.success(
        "Application is fully synchronized with database-driven internship"
        " recommendations, PDF parsing, AI reasoning, and deployed"
        " successfully!"
    )

    st.markdown("### 🗺️ Personalized Learning Roadmap")
    sample_missing = [
        "PyTorch Advanced Optimization",
        "Deployment with Streamlit Cloud",
    ]
    roadmap = generate_learning_roadmap(sample_missing)
